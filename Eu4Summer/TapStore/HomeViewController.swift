...
class HomeViewController: UIViewController, UICollectionViewDelegate {
    //Riferimento a link o file .jason da cui ricavare oggetti da inserire nell'app
    ...
    private let networkinClient = NetworkingClient()

    override func viewDidLoad() {
        super.viewDidLoad()
        
         let url = URL(string: "https://api.myjson.com/bins/7arjq")!
       
          let data = try! Data.init(contentsOf: URL.init(string: "https://api.npoint.io/2a364664e19dd7d769d5")!)
        ...
        buildPreMenu()
        networkinClient.execute(url: url) { (json, error) in
            if let error = error {
                print(error)
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        print(sections[0].items[indexPath.row].id)
        idForSegue = sections[0].items[indexPath.row].id
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "details" {
            let destinationVC = segue.destination as! DetailsViewController //Fatto, era questo
           destinationVC.id = self.idForSegue
        }
    }
